...
function callDeepSeekAPI(messages, model = DEFAULT_MODEL) {
...
async function executeTool(command) {
    ...
    if (trimmed.startsWith('ls ') || trimmed.startsWith('dir ') || trimmed === 'ls' || trimmed === 'dir') {
        ...
        try {
            ...
            const result = (0, child_process_1.execSync)('ls -la "' + target + '"', { cwd, encoding: 'utf-8', timeout: 10000 });
            return result;
        }
        catch (e) {
    ...
    if (trimmed.startsWith('grep ') || trimmed.startsWith('rg ')) {
        try {
            const cwd = process.cwd();
            const result = (0, child_process_1.execSync)(command, { cwd, encoding: 'utf-8', timeout: 10000 });
            return result;
        }
        catch (e) {
            return 'Error: ' + e.message;
...
async function startChat(model) {
    ...
    console.log(chalk_1.default.bold.cyan('+------------------------------------------------------------+'));
    console.log(chalk_1.default.bold.cyan('|') + chalk_1.default.bold.white('            Coding AI (Read-only Mode)               ') + chalk_1.default.bold.cyan('|'));
    console.log(chalk_1.default.bold.cyan('+------------------------------------------------------------+'));
    console.log(chalk_1.default.gray('  模型: ' + (model || DEFAULT_MODEL)));
    console.log(chalk_1.default.gray('  输入问题或命令，或 \'exit\' 退出\n'));
    console.log(chalk_1.default.bold.cyan('------------------------------------------------------------'));
    while (true) {
        ...
        try {
            ...
            console.log(chalk_1.default.red('❌ API 错误: ' + e.message));
