...
export async function selectInstallTarget(): Promise<InstallTarget | null> {
  ...
  await interactiveSelect({
    title: '选择安装类型',
    options: [
      { label: '下载AI CLI工具', value: 'cli', description: 'Claude Code, Kimi, Codex, Kiro...' },
      { label: 'AI IDE', value: 'ide', description: 'VS Code, Cursor, TRAE, Qoder...' },
      { label: '魔法环境工具', value: 'environment', description: '代理环境、虚拟卡、指纹浏览器' },
    ],
    onSelect: (value) => {
      category = value;
    },
  ...
  if (category === 'ide') return selectTarget('选择 AI IDE', IDE_INSTALL_TARGETS);
