 ...
 * pipeline from user input to first token arrival. Enable by setting UPSTREAM_AGENT_PROFILE_QUERY=1
 *
...
const ENABLED = isEnvTruthy(process.env.UPSTREAM_AGENT_PROFILE_QUERY)
...
function getQueryProfileReport(): string {
  if (!ENABLED) {
    return 'Query profiling not enabled (set UPSTREAM_AGENT_PROFILE_QUERY=1)'
  }
