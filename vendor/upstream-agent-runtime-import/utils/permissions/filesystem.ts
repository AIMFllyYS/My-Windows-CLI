...
// Always ask when Reference Agent tries to edit its own config files
 ...
 * This is a per-user temporary directory used by Reference Agent for all temp files.
 *
...
// and per-turn from BashTool prompt. Inputs (UPSTREAM_AGENT_TMPDIR env + platform) are
...
export const getClaudeTempDir = memoize(function getClaudeTempDir(): string {
  const baseTmpDir =
    process.env.UPSTREAM_AGENT_TMPDIR ||
 ...
 * - MCP CLI state files (managed internally by Reference Agent)
