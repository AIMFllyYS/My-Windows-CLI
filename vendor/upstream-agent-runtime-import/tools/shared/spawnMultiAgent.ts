...
async function handleSpawnSplitPane(
  ...
  // Build the command to spawn Reference Agent with teammate identity
  // Note: We spawn without a prompt - initial instructions are sent via mailbox
  const binaryPath = getTeammateCommand()

  // Build teammate identity CLI args (replaces UPSTREAM_AGENT_* env vars)
  const teammateArgs = [
  ...
  // Includes REFERENCEAGENT, UPSTREAM_AGENT_EXPERIMENTAL_AGENT_TEAMS, and API provider vars.
...
/**
 * Handle spawn operation - creates a new Reference Agent instance.
